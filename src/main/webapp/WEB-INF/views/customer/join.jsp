	...
	<div class="container" id="container">
		...
		<div class="form-container sign-in-container">
			<form action="${pageContext.request.contextPath}/customer/join"
				...
				<!------------------------------- 인풋 영역 ---------------------------------------->
				<!-- 아이디 유효성 체크 html 요소 -->
				<span id="checkId"></span> <input type="text" placeholder="Id"
					id="inputId" name="customerId" required> <input type="text"
					placeholder="name" id="" name="customerName" required /> <input
					type="password" id="inputPassword" placeholder="Password"
					name="customerPassword" required /> <a href="#"></a>
			...
			<div class="overlay">
				...
				<div class="overlay-panel overlay-right">
					...
					<a class="btn btn-outline-light btn-lg "
						href="${pageContext.request.contextPath}/customer/loginPage">
						로그인 페이지로 이동하기 </a>
				</div>
			</div>
		</div>
...
<script>
	window.onload = function() {
		...
		$('#joinForm').on("submit", function() {
			...
			if (idCheck == '이미 존재하는 아이디입니다.') {
				showModal('다른 아이디로 가입해주세요.');
				return false;
			} else if (inputPassword.length < 3) {
				showModal('비밀번호는 3자리 이상부터 가능합니다.');
				return false;
			} else if (inputId.length < 3) {
				showModal('아이디는 3자리 이상부터 가능합니다.');
				return false;
			}
